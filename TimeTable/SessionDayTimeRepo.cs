    ...
    class SessionDayTimeRepo
    {
        ...
        public List<SessionDayTime> GetAll()
        {
            ...
            using (var connection = factory.CreateConnection())
            {
                ...
                command.CommandText = "Select * From Session_Time;";
                using (DbDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        {
                            Sessiontime_ID = (int)reader["Sessiontime_ID"],
                            Lecturer_Name_1 = (string)reader["Lecturer_Name_1"],
                            Lecturer_Name_2 = (string)reader["Lecturer_Name_2"],
        ...
        public void Add(SessionDayTime SessionDayTime)
        {
            using (var connection = factory.CreateConnection())
            {
                ...
                var command = factory.CreateCommand();
                command.Connection = connection;
                command.CommandText = $"Insert into Session_Time(Lecturer_Name_1,Lecturer_Name_2,Subject_Code,Subject_Name,Group_ID,Tag,Room,Day,Start_Time,End_Time) Values('{SessionDayTime.Lecturer_Name_1}','{SessionDayTime.Lecturer_Name_2}','{SessionDayTime.Subject_Code}','{SessionDayTime.Subject_Name}','{SessionDayTime.Group_ID}','{SessionDayTime.Tag}','{SessionDayTime.Room}','{SessionDayTime.Day}','{SessionDayTime.Start_Time}','{SessionDayTime.End_Time}');";
